...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const checkForLocalConfig = async () => {
    ...
    try {
      ...
      console.log(
        `${MESSAGE_PREFIX}Error while trying to checkForLocalConfig. ${JSON.stringify(
          _ignorred
        )}`
      );
    } finally {
  ...
  const findRepositoryRoot = async (showError = true) => {
    ...
    console.log(`rootBaseDir is ${rootBaseDir}`);
  ...
  const getGitConfig = async (repository: Repository, showMessages = true) => {
    try {
      const userEmail = (
        await repository.configGet("local", "user.email", {})
      ).trim();
      const userName = (
        await repository.configGet("local", "user.name", {})
      ).trim();
      showMessages &&
        vscode.window.showInformationMessage(
          `${MESSAGE_PREFIX}user.email=${userEmail} user.name=${userName}`
        );
      ...
      showMessages &&
        vscode.window.showWarningMessage(
          `${MESSAGE_PREFIX}user.email or user.name is not set locally. You can set it using command '' `
        );
    }
    return null;
  };
  ...
  const setGitConfig = async () => {
    ...
    if (configList.length) {
      const map: Map<string, GitConfig> = configList
        .concat(CUSTOM_GIT_CONFIG, IGNORE_CURRENT_ROOT_GIT_CONFIG)
        .reduce((map, c) => {
      ...
      const pick = await vscode.window.showQuickPick(Array.from(map.keys()), {
        ignoreFocusOut: true,
        placeHolder:
          "Select one of previous configs or new custom one or ignore current root.",
      });
      if (pick === generateGitConfigKey(CUSTOM_GIT_CONFIG)) {
        await customSetGitConfig();
      } else if (
        pick === generateGitConfigKey(IGNORE_CURRENT_ROOT_GIT_CONFIG)
      ) {
  ...
  const ignoreRootCommand = vscode.commands.registerCommand(
    COMMAND_IGNORE_ROOT,
    async () => {
  ...
  const unignoreRootCommand = vscode.commands.registerCommand(
    COMMAND_UNIGNORE_ROOT,
    async () => {
